...
export function ImageConverterEn() {
  ...
  // Calculate space saving statistics
  useEffect(() => {
  ...
  // Function to download all converted images as a ZIP file
  const downloadAllAsZip = async () => {
    ...
    try {
      ...
      const fetchPromises = completedFiles.map(async (file) => {
        ...
        // Add to ZIP with an appropriate filename
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Supports PNG, JPG, JPEG, GIF, BMP, TIFF</p>
        </div>
      </div>
              ...
              {hasCompletedFiles && (
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>Download all converted images as a ZIP file</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )}
          ...
          {hasCompletedFiles && totalSaved.original > 0 && (
            <div className="bg-teal-50 dark:bg-teal-900/20 rounded-lg p-4 border border-teal-100 dark:border-teal-900/50">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-medium text-teal-800 dark:text-teal-300">Savings Statistics</h4>
                  <div className="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      <p className="text-slate-500 dark:text-slate-400">Converted Size</p>
                      <p className="font-medium text-slate-900 dark:text-white">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {file.status === "completed" && (
                      <div className="flex items-center gap-2 bg-teal-50 dark:bg-teal-900/20 px-3 py-1 rounded-full">
                        ...
                        <span className="text-sm text-teal-700 dark:text-teal-300">Converted</span>
                        <span className="text-xs bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full">
                          ...
                          {calculateSavings(file.originalSize || 0, file.convertedSize || 0)} saved)
                        </span>
                      </div>
                    )}
                    {file.status === "error" && (
